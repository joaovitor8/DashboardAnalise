...
def clean_data(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
  ...
  # ── Colunas sem nome (Unnamed: 0, Unnamed: 1, ...) ───────────────────
  sem_nome = [col for col in df.columns if str(col).startswith("Unnamed")]
  if sem_nome:
    df.drop(columns=sem_nome, inplace=True)
    mensagens.append(f"Colunas sem nome removidas: {', '.join(sem_nome)}")

  # ── Colunas com nulos críticos (≥99%) → remove antes de qualquer coisa
  colunas_removidas = []
  ...
  # ── Nulos numéricos → média (com guarda para NaN) ────────────────────
  ...
  # ── Nulos categóricos → "Não informado" ──────────────────────────────
